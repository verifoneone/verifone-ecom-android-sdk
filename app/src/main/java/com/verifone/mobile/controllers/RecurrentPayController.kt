...
class RecurrentPayController(ctx: Context,brand:String,onReuseTokenCallback: (api_response: String) -> Unit) {
    ...
    private var resultHandler : io.reactivex.Observer<ReuseTokenResponse> = object:
        ...
        override fun onNext(valueResponse: ReuseTokenResponse) {
            ...
            saveCardDetails(valueResponse.reuseToken,cardBrand)
    ...
    fun getReuseTokenRequest(encryptedCardParam:String,tokenScopeParam:String,publicKeyAliasParam:String,tokenTypeParam:String,
                             tokenExpiry:String) {
        val mRetrofitClient =
    ...
    private fun saveCardDetails(token:String,brand:String) {
        val sp = mCtx.getSharedPreferences("checkout_data",Context.MODE_PRIVATE)
        sp.edit()
            .putString("key_store_rec",token)
            .putString("key_store_brand",brand)
            .apply()
    }
